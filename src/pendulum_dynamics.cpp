...
void PendulumDynamics::rungeKutta() {
  ...
  for (int i = 0; i < 4; ++i) {
    _updatedStates[i] =
        receivedStates[i] + (h * ((K1[i] + 2 * K2[i] + 2 * K3[i] + K4[i]) / 6));
  }
...
std::array<double, 4> PendulumDynamics::f(std::array<double, 4> _states,
  ...
  if (controllerState) {
    ...
    tau1 = kp * (_u[0] - _states[0]) - kd * (_states[2]) +
           ((m1 + m2) * l1 * sin(_u[0]) + m2 * l2 * sin(_u[0] + _u[1])) * g;
...
PendulumDynamics::inverseKinematics(std::array<double, 2> _pos) {
  ...
  // calculating inverse kinematics always in quadtrant 1 and transforming to
  // other quadrants!
  if (x >= 0 && y >= 0) {
...
// another _over to account for the multiplication with h/2 or h in the runge
// kutta steps
std::array<double, 4> PendulumDynamics::addArrays(std::array<double, 4> _x,
                                                  std::array<double, 4> _K,
                                                  double _over) {
