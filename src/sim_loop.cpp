...
void SimLoop::run() {
  ...
  while (!helperVars.getQuit()) {
    ...
    if (pause) {
      ...
      x_conv =
          x * 1 /
          (l1 + l2); // total length of robot arm over total length in pixels
      y_conv = -y * 1 / (l1 + l2);
      ...
      // when mouse is outside of workspace
        ...
        if (x_conv < 0) {
          xd = -xd;
          yd = -yd; // flipping for quadrant 2 and 3
        }
      }

      // integration// two integration steps per frame
      for (int i = 0; i < 10; ++i) {
        pendulumDynamics.setReceivedInputs(
            pendulumDynamics.inverseKinematics({xd, yd}));
      ...
      x2 = x1 +
           l2 * sin(pendulum.getStates().at(0) + pendulum.getStates().at(1));
      y2 = y1 +
           l2 * cos(pendulum.getStates().at(0) + pendulum.getStates().at(1));
    }

    helperVars.getTrajectory().push_back(
        {x2, y2}); // store endeffector position in trajectory

    // rendering screen
    ui.clear(); // clears screen

    int width = 10; // half width of the link
    // angles of the short side of the link to the origin (makes it a bit easier
    // to calculate)
    double ang1 = pi / 2 - pendulum.getStates().at(0);
    double ang2 =
        pi / 2 - (pendulum.getStates().at(0) + pendulum.getStates().at(1));
